...
def collect_market_ids(event_ids: List[int], max_workers: int = 10, batch_size: int = 5) -> Dict[int, List[int]]:
    """
    Concurrently collect market IDs for all events with improved error handling
    ...
    # Convert all event_ids to strings and remove any None or invalid values
    event_ids = [str(eid) for eid in event_ids if eid is not None]
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        total_batches = len(event_id_batches)
        completed_batches = 0
        ...
        for future in as_completed(future_to_batch):
            try:
                ...
                # Update progress
                completed_batches += 1
                if completed_batches % 10 == 0:  # Print progress every 10 batches
                    print(f"Processed {completed_batches}/{total_batches} batches "
...
def process_market_data(
    ...
    """
    Concurrently process market data for all market IDs with improved error handling
    ...
    """
    # Convert all market_ids to strings and remove any None or invalid values
    market_ids = [str(mid) for mid in market_ids if mid is not None]
    ...
    # Track progress
    total_batches = len(market_id_batches)
    completed_batches = 0
...
def fetch_market_ids_for_event_batch(batch_ids: List[int]) -> Dict[int, List[int]]:
    """
    Fetch market IDs for a small batch of event IDs with improved error handling
    """
    headers = get_headers()
    event_market_ids_dict = {}
    max_retries = 3
    ...
    for attempt in range(max_retries):
        try:
            # Increased timeout and added exponential backoff
            time.sleep(random.uniform(0.5 * (attempt + 1), 1.0 * (attempt + 1)))
            
            response = requests.get(url, headers=headers, timeout=60)
            if response.status_code != 200:
                print(f"Attempt {attempt + 1}: HTTP {response.status_code} for event_ids: {batch_ids}")
                continue
                
            json_data = response.json()
            
            for event in json_data.get('data', {}).get('events', []):
                event_id = event['id']
                market_ids = []
                
                collections = event.get('collections', [])
                
                # First try preferred collections
                ...
                # If no markets found, try fallback collections
                if not market_ids:
                    for collection in collections:
                        if collection['name'] in ['Wedstrijd', 'Doelpunten', 'Schoten', 'Schoten op doel']:
                            market_ids = collection.get('marketIds', [])
                            if market_ids:
                                break
                
                if market_ids:  # Only add if we found market IDs
                    event_market_ids_dict[event_id] = market_ids
            ...
            print(f"Timeout on attempt {attempt + 1} for event_ids: {batch_ids}")
            if attempt == max_retries - 1:
                print(f"Max retries reached for event_ids: {batch_ids}")
        except Exception as e:
            print(f"Error on attempt {attempt + 1} for event_ids: {batch_ids}: {str(e)}")
            if attempt == max_retries - 1:
                print(f"Max retries reached for event_ids: {batch_ids}")
...
def fetch_market_data_for_batch(batch_market_ids: List[int]) -> List[Dict[str, Any]]:
    """
    Fetch market data for a batch of market IDs with improved error handling
    """
    headers = get_headers()
    batch_data = []
    max_retries = 3
    ...
    for attempt in range(max_retries):
        try:
            ...
            response = requests.get(url, headers=headers, timeout=60)
            if response.status_code != 200:
                print(f"Attempt {attempt + 1}: HTTP {response.status_code} for batch")
                continue
                    ...
                    for outcome in market.get('outcomes', []):
                        ...
                        try:
                            batch_data.append({
                                ...
                                'Outcome Name': outcome.get('name', ''),
                                'Odds (Decimal)': prices[0].get('decimal', None),
                                'Price Numerator': prices[0].get('numerator', None),
                                'Price Denominator': prices[0].get('denominator', None),
                                'Outcome Type': outcome.get('type', ''),
                                'Outcome SubType': outcome.get('subType', '')
                            })
                        except (IndexError, KeyError) as e:
                            print(f"Error processing outcome data: {str(e)}")
                            continue
            ...
            print(f"Timeout on attempt {attempt + 1} for batch")
            if attempt == max_retries - 1:
                print(f"Max retries reached for batch")
        except Exception as e:
...
def main() -> Dict[str, pd.DataFrame]:
    """
    Main function with improved error handling and smaller batch sizes
    """
    # Step 1: Get event matches
    print("Toto: Fetching matches...")
    matches_df = get_event_matches()
    
    # Step 2: Collect market IDs with smaller batch size
    ...
    # Step 4: Process market data with smaller batch size
    print("Toto: Fetching odds data from Market IDs...")
    market_data_df = process_market_data(
        all_market_ids, 
        max_workers=50,   # Reduced from 100
